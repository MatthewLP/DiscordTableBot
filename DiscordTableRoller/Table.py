'''Defines the Table class for used in the Table Bot'''
import csv
import random
import io
...
class Table:
    ...
    def __init__(self, filepath, container={}, parents=[]):
        ...
        self._table_build(contents, name_index, used_columns)

    def _table_build(self, contents, name_index, used_columns):
        '''Builds the self.data dictionary using the columns of contents defined by
used_columns using the names (keys) found in the column name_index'''
                ...
                if contents[1][i] == 'recur':
                    row_lst.append(Table(\
                        path.normpath(path.join( \
                            path.dirname(self.filepath), row[i])), \
                        parents=self.parents + [self.filepath]))
                else:
    ...
    def query(self, name: str, datum_name=None, *rcrsv_call_data):
        ...
        if name in self.data:
            if datum_name is None:
                out_lst.extend(self._full_describe(name))
    ...
    def _full_describe(self, name):
        '''fully describes 'name' listing all non-special columns followed by the
description or brief depending on which exists, then a list of recursive
options if avalable. Returns a list of strings markdown formated.'''
        out_lst = ['```\n', name, ':\n']
    ...
    def get_item_names(self):
        'returns an itterable to the names of rows'
